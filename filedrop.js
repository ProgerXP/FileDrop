...
;(function (global, exports) {
  ...
  // Internet Explorer 9+ only (it doesn't support File API earlier anyway but
  // make sure not to call this from your scripts).
  ...
  global.extend(global, {
    ...
    // Test for IE version, must be true for 6-7 and below and
  ...
  global.DropHandle = function (zone, opt) {
    ...
    self.hookDropOn = function (zoneNode) {
      // IE is as amusing as always. IE 6 fires 'drop' event, probably was the
      // first in its class. However, if used IE will send an empty POST body
      // instead of the actual file data. So handling of this event is disabled
      // although technically it could've worked if not for that bug (?).
      // And the amusing thing is that IE 8 itself no more responds to this event.
      //
      ...
      global.isIE6 || self.delegate(zoneNode, 'drop', 'upload')
  ...
  global.File = function (file) {
    ...
    self.events = {
      ...
      //      while reading file from the local file system with readAsBinaryString().
      error: []
    }
    ...
    // upload. For FileAPI (Firefox) gets called on readAsBinaryString() onload
    // event; for Safari/early Chrome it's called immediately and
    // gets passed the native file object itself.
    ...
    //      // browser reads (earlier Chrome required read values to be normalized
    //      // with toBinary()).
    //
